...
from .graph import Graph, GraphWithAnswer, IndexedGraph
from .graph import relabel_nodes
...
def gen_ans_by_masking(graph: Graph, num_nodes, relation_cnt, args) -> GraphWithAnswer:
    device = graph.edge_index.device
    mask_ratio = args.pretrain_mask_ratio
    mask_type_ratio = args.pretrain_mask_type_ratio
    ...
    def do_mask(arr, list_predict, num_nodes) -> torch.LongTensor:
        ...
        if randseed < args.mask_ratio:
            arr[list_predict] = -1
        elif args.mask_ratio <= randseed < (args.mask_ratio + 1) / 2:
            arr[list_predict] = torch.randint(low=0, high=num_nodes, size=list_predict.shape, device=device)
        else:
            pass
        return arr
    x_node = do_mask(graph.x, x_query, num_nodes)
...
def sample_n_p(igraph: IndexedGraph, target_node, n, args):
    dfn = {target_node: 0}
    idx = [target_node]
    ...
    randseed = random.random()
    if randseed <= args.p_mask_ratio:
        x_node = torch.tensor([-1] * n + [idx[-1]])
        ans = torch.tensor([target_node])
    elif args.p_mask_ratio < randseed <= (args.mask_ratio + 1) / 2:
        x_node = torch.tensor([-1] * n + [idx[-1]])
    ...
    result = GraphWithAnswer(
        x=x_node,
        x_d=torch.zeros_like(x_node, dtype=torch.long),
        r_d=torch.zeros_like(result.edge_attr, dtype=torch.long),
...
def sample_n_i(igraph: IndexedGraph, target_node, n, edge_drop_out_rate):
    ...
    x_node = torch.tensor([-1] + adj_node)
    g = GraphWithAnswer(
        x=x_node,
        x_d=torch.zeros_like(x_node, dtype=torch.long),
        r_d=torch.zeros_like(g.edge_attr, dtype=torch.long),
...
def mini_sampler(igraph: IndexedGraph, target_nodes, lap_matrix, relation_cnt, args) -> GraphWithAnswer:
    
    target_node = np.random.choice(target_nodes)
    d: dict = args.pretrain_sampler_ratio
    edge_drop_out_rate = args.edge_drop_out_rate
    a = list(d.keys())
    p = np.array(list(map(d.__getitem__, a)), dtype=np.float32)
    p = p / p.sum()
    g_type = np.random.choice(a=a, p=p)
    g = None
    for n in range(args.sample_retries):
        if g_type == '1p':
            g = sample_n_p(igraph, target_node, 1, args)
        elif g_type == '2p':
            g = sample_n_p(igraph, target_node, 2, args)
        elif g_type == '3p':
            g = sample_n_p(igraph, target_node, 3, args)
        elif g_type == '2i':
            ...
            g = sample_meta_tree(target_node, igraph, edge_add_ratio=args.induced_edge_prob)
        elif g_type == 'ladies':
            g = sample_from_origin(
                node_list=target_nodes,
                igraph=igraph,
                lap_matrix=lap_matrix,
                ladies_size=args.ladies_size,
                induced_edge_prob=args.induced_edge_prob,
            )
        else:
