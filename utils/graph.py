...
class Graph(torch_geometric.data.Data):
    num_nodes: int  # 实体数量E
    num_edges: int  # 边数量
    edge_index: torch.LongTensor  # torch.Size([2, E]), the directed edges [[],[]] 头实体列表，尾实体列表
...
class GraphWithAnswer(Graph):
    x_d: torch.LongTensor 
    r_d: torch.LongTensor 
    ...
    def __inc__(self, key, value, *args, **kwargs):
        if key == 'x_query':
            ...
            return torch.tensor([[len(self.x_query)], [0]])
        else:
...
class MatGraph:
    ...
    def __init__(self, x, x_d, embed_type, pred_type, x_query=None, x_ans=None, x_pred_weight=None, attn_bias_type=None,
    ...
    def make_line_graph(g: GraphWithAnswer, relation_cnt: int):
        ...
        x = cat([g.x, edge_attr]) # 实体+关系 （将关系也转成节点）
        x_d = cat([g.x_d, g.r_d, g.r_d]) # 节点的重要程度
        embed_type = cat([full(g.x.shape, 0), full(edge_attr.shape, 2)]) # 表示x的类型 0表示实体，2表示关系，1表示mask
        mask = x == -1
        x[mask] = 0
        embed_type[mask] = 1
        ...
        edge_nodes = g.num_nodes + arange(g.num_edges * 2) # [0,1] + 2 = [2,3] 在实体节点基础上，将关系节点重新编号
        add_edge(all_nodes, all_nodes)  # self-attention
        add_edge(edge_index[0], edge_nodes)  # 将头实体节点和边节点 添加一条边
        add_edge(edge_nodes, edge_index[1]) # 将边节点和尾实体节点 添加一条边 
        return MatGraph(
            x=x,
            x_d=x_d,
...
class BatchMatGraph(DictObject):
    ...
    def from_mat_list(arr):
        ...
        # print('x', b.x.shape)
        # print('x_d',b.x_d.shape)
        # print('embed_type',b.embed_type.shape)
        # print('pred_type',b.pred_type.shape)
        # print('attn_bias_type',b.attn_bias_type.shape)
        return b
...
def get_directed_lap_matrix_np(edge_index, num_nodes):
    edge_index = to_undirected(edge_index=edge_index, no_dup_self_loops=False) # T/2 ->T
    import scipy.sparse as sp
    adj = get_adj_np(edge_index, torch.ones(edge_index.shape[1], dtype=torch.float), num_nodes) # 邻接矩阵，采用稀疏矩阵形式表示
    adj = row_normalize_np(adj + sp.eye(num_nodes, format='csr')) # 按行归一化
    return adj
...
def to_undirected(edge_index: torch.Tensor, no_dup_self_loops=True) -> torch.Tensor:
    ...
    """
    r_edge_index = edge_index[[1, 0]] # (2, triple_nums/2)
    if no_dup_self_loops:
        mask = edge_index[0] != edge_index[1]
        r_edge_index = r_edge_index[:, mask]
    edge_index = torch.cat([edge_index, r_edge_index], dim=1) # (2, triple_nums)
    return edge_index
    ...
    def get_induced_subgraph(self, node_list) -> Graph:
        ...
        if len(cat_list0) == 0:
            return Graph(
                ...
                edge_attr=torch.tensor([], dtype=self.edge_attr.dtype),
            )
        return Graph(
            x=self.x,
