...
import numpy as np
import torch
from utils.metric import loss_cross_entropy_multi_ans, loss_label_smoothing_multi_ans
...
class GaussianDiffusion:
    ...
    def __init__(self, args, betas):
        ...
        self.predict_xstart = args.predict_xstart # # model is used to predict xstart or eps

        self.model_mean_type = "xstart" #"eps"
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps): # 根据xt和eps，预测x0，即xstart
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart): # 根据xt和x0，预测eps
        return (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
            - pred_xstart
    ...
    def q_sample(self, x_start, t, noise=None, mask=None):
        ...
        """
        if noise is None:
            noise = torch.randn_like(x_start) # noise(batch, len, dim) r_embedding, (batch,dim)然后将它扩展len
        ...
        if mask == None:
            ...
            return torch.where(mask==0, x_start, x_t) # query+answer，只将anwer加噪声
    ...
    def _token_discrete_loss(self, x_t, get_logits, input_ids, node_type, mask=None, truncate=False, t=None):
        '''
        ...
        reshaped_x_t = x_t.view(input_ids.shape[0], -1)
        logits = get_logits(node_type, reshaped_x_t)

        decoder_nll = torch.zeros(input_ids.shape[0], device=input_ids.device)
        for i in range(3):
            node_mask = node_type == i
            decoder_nll[node_mask] += loss_fct(logits[i][node_mask], input_ids[node_mask])
        if mask != None:
            decoder_nll *= mask
        if mask != None:
    ...
    def training_losses_seq2seq(self, model, data, t, weights, noise=None):
        """
        ...
        batch_size, seq_len = data.num_graphs, data.num_nodes_per_graph
        x_start_mean = model.model.token_embed(data.embed_type, data.x) 
        x_start_mean = x_start_mean.view(batch_size, seq_len, -1)
        ...
        # x_mask = data.pred_type.view(batch_size, seq_len)
        x_mask=None
        x_t = self.q_sample(x_start, t, noise=noise, mask=x_mask) 

        target = {
            'xprev': self.q_posterior_mean_variance(x_start=x_start, x_t=x_t, t=t)[0],
            'xstart': x_start,
            'eps': noise}[self.model_mean_type]
        
        model_output, _ = model(
            x_t, self._scale_timesteps(t), 
            attn_bias_type=data.attn_bias_type, 
            pred_type=data.pred_type,
            node_type=data.embed_type, 
            node_degree=data.x_d,
            isdiffu=True) 
        ...
        mse = torch.where(t0_mask, t0_loss, mse)

        # tT_mask = (t == self.num_timesteps - 1)
        out_mean, _, _ = self.q_mean_variance(x_start, torch.LongTensor([self.num_timesteps - 1]).to(device))
        tT_loss = mean_flat(out_mean ** 2)

        _, score = model(model_out_x_start, self._scale_timesteps(t), 
                         attn_bias_type=data.attn_bias_type, pred_type=data.pred_type, isdiffu=False) # (BE,N_KG) 
        ...
        if model.model.loss_type == "CE":
            kg_loss, kg_weights = loss_cross_entropy_multi_ans(
                score.double(), 
        ...
        elif model.model.loss_type == 'LS':
            kg_loss, kg_weights = loss_label_smoothing_multi_ans(
                score.double(),
                relabel_arr[data.x_query[q_mask]], data.x_ans[q_mask],
                relabel_arr[data.pos_x[p_mask]], data.pos_ans[p_mask],
        ...
        d_loss = mse + tT_loss
        # d_loss = mse + tT_loss + decoder_nll
        d_loss = (d_loss * weights).mean()
        kg_loss = (kg_loss * kg_weights).mean()
        # losses = kg_loss
        losses = d_loss + kg_loss
        return losses
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, args, betas, use_timesteps):
        self.use_timesteps = set(use_timesteps)
        self.timestep_map = []
        self.original_num_steps = len(betas)

        base_diffusion = GaussianDiffusion(args, betas)  # pylint: disable=missing-kwoa
        ...
        args.betas = np.array(new_betas)
        super().__init__(args, betas)
